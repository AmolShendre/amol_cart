...
switch ($_POST['req']) {
  ...
  case "show":
    ...
    <table id="scTable">
      ...
      <tr>
        ...
        <td>  <img class="pImg_checkout" src="public/<?= $products[$id]['product_image'] ?>"/>    <?= $products[$id]['product_name'] ?></td>
    ...
    <form id="scCheckout" onsubmit="return cart.checkout();">
      <h1>CHECKOUT</h1>
      <label>Name</label>
      <input type="text" id="co_name" required value="Amol Shendre"/>
      <label>Email</label>
      <input type="email" id="co_email" required value="amol@shendre.com"/>

      <label>Phone</label>
      <input type="number" id="co_no" required value="1234512345"/>

      <label>Address</label>
      <input type="text" id="co_address" required value="Nagpur"/>


      <input type="submit" class="bRed" value="Place Order"/>
    </form>
    <?php }
    break;
  ...
  case "checkout":
    $pass = $_CC->cartCheckout($_POST['name'], $_POST['email'],$_POST['phone'],$_POST['address']);
    echo json_encode([
      "status" => $pass,
      "message" => $pass ? "Order confirmed....THANK-YOU ..!" : $_CC->error
    ]);
    break;
}